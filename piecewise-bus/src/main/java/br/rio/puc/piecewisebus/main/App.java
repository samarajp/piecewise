...
//			cost = datafunction.yFinal;
		...
		//	grafo.addAresta(source, target, element.getEdge().trim(), cost);
			...
			grafo.addAresta(source, target, "1", cost);
...
//		System.out.println("Custo da edge que será atualizada: "+cost);
	...
	//	Set<Vertice> path = dijkstra.findShortest("s");
	    Set<Vertice> path = dijkstra.findShortest("0");


		Iterator<Vertice> it = path.iterator();
		    while (it.hasNext()) {
		        System.out.println("chamando o dijkstra: "+it.next().getNome());
		    }
